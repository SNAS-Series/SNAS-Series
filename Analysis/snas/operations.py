...
OPS = {
    ...
    'conv_1x1': lambda C, stride, affine: Conv1x1(C, C, 1, stride, 0, affine=affine),
    'conv_3x3': lambda C, stride, affine: Conv3x3(C, C, 3, stride, 1, affine=affine),
    'conv_5x5': lambda C, stride, affine: Conv5x5(C, C, 5, stride, 2, affine=affine),
    ...
    'conv_7x1_1x7': lambda C, stride, affine: nn.Sequential(
        ...
        nn.BatchNorm2d(C, affine=affine, momentum=0)
    ),
}
...
class AvgPool(nn.Module):
    def __init__(self, C, stride, padding, count_include_pad=False, affine=True):
        ...
        self.op = nn.Sequential(
            ...
            nn.BatchNorm2d(C, eps=1e-5, affine=affine, track_running_stats=True)
            #nn.InstanceNorm2d(C, atchNorm2dffine=True)
        )
        #self.op[1].bias.requires_grad=False
        #nn.init.constant_(self.op[1].bias, 1)
        self.size = 0
        self.flops = 0
        self.mac = 0
...
class ReLUConvBN(nn.Module):
    def __init__(self, C_in, C_out, kernel_size, stride, padding, affine=True):
        ...
        self.op = nn.Sequential(
            nn.ReLU(inplace=False),
            nn.Conv2d(C_in, C_out, kernel_size, stride=stride, padding=padding, bias=False),
            nn.BatchNorm2d(C_out, eps=1e-5, affine=affine, track_running_stats=True)
            #nn.InstanceNorm2d(C_out, affine=True)
        )
        #nn.init.constant_(self.op[2].bias, 1)
        #self.op[2].bias.requires_grad=False
        self.size = C_in * C_out * kernel_size * kernel_size
        self.flops = 0
        self.mac = 0
...
class DilConv(nn.Module):
    def __init__(self, C_in, C_out, kernel_size, stride, padding, dilation, affine=True):
        ...
        #nn.init.constant_(self.op[3].bias, 1)
        #self.op[3].bias.requires_grad=False
...
class SepConv(nn.Module):
    def __init__(self, C_in, C_out, kernel_size, stride, padding, affine=True):
        ...
        self.op = nn.Sequential(
            ...
            nn.BatchNorm2d(C_in, eps=1e-5, affine=affine, track_running_stats=True),
        ...
        #nn.init.constant_(self.op[3].bias, 1)
        #self.op[3].bias.requires_grad=False
        #self.op[7].bias.requires_grad=False
        #nn.init.constant_(self.op[7].bias, 1)
...
class FactorizedReduce(nn.Module):
    def __init__(self, C_in, C_out, affine=True):
        ...
        self.bn = nn.BatchNorm2d(C_out, eps=1e-5, affine=affine, track_running_stats=True)
        #self.bn = nn.InstanceNorm2d(C_out, affine=True)
